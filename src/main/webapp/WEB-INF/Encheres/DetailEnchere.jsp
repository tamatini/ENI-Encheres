...
<main class="darkblue_main">
  <section class="">
    <div class="auction_article_form container">
      <div>
        <c:choose>
          <c:when test="${dateAujourdhui < dateDebutEnchere}">
           <h1 class="auction_title">L'enchère n'a pas encore commencée</h1>
          </c:when>
          <c:when test="${dateAujourdhui >= dateFinEnchere && empty enchere.acheteur}">
           <h1 class="auction_title">L'enchère est terminée</h1>
          </c:when>
          <c:when test="${dateAujourdhui >= dateFinEnchere && user.utilisateurId == enchere.acheteur.utilisateurId}">
           <h1 class="auction_title">Vous avez remporté la vente</h1>
          </c:when>
          <c:when test="${dateAujourdhui >= dateFinEnchere && user.utilisateurId != enchere.acheteur.utilisateurId}">
           <h1 class="auction_title">${enchere.acheteur.pseudo} a remporté l'enchère</h1>
          </c:when>
          <c:otherwise>
           <h1 class="auction_title">Détail vente</h1>
          </c:otherwise>
        </c:choose>
      </div>
    ...
    <div class="container flex_cards">
      <c:if test="${dateAujourdhui >= dateFinEnchere &&
                  user.utilisateurId == enchere.article.vendeurId &&
                  enchere.article.prixVente == 0 &&
                  ! empty enchere.acheteur.utilisateurId}">
